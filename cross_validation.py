...
from linear_regression_with_gradient import calulations, target_attribute, calulations_with_regularization
...
class Validator:
  ...
  def start_validation(self, frame):
    ...
    for train_ind, test_ind in sets.split(columns_without_target):
      ...
      training_prediction, θ = self.calculations(train_bias, train_column,self.rate, self.iterations, self.batch_size, None, None,reg_type='l2', lambda_=0.01)
...
def main():
  ...
  validator1 = Validator(calulations_with_regularization,processor,target_attribute)
  #validator.start_validation(frame)
  validator1.start_validation(frame)
